...
folder = dataiku.Folder('oman2QQ8')
all_paths = folder.list_paths_in_partition()

# Initialize datasets
output_data = dataiku.Dataset("DBC2")
log_data = dataiku.Dataset("DBL2")
...
def process_file(path):
    ...
    short_thread_id = str(thread_id)[-6:]
    ...
    try:
        ...
        elif extension in ['.pptx', '.xlsx', '.docx', '.doc']:
        ...
        try:
            ...
            # Create configuration dictionary - THIS IS CRITICAL
            gemini_config = {
                'endpoint': gemini_endpoint,
                'api_key': API_KEY
            }
            
            chunks, pages, tokens, price = extract_image_pdf_gemini(
                pdf_path, dirname, basename, gemini_config, model_name, price_per_mil_token
            )
